...
- **Recommended:** Monk's Active Tile Triggers `12.01–12.02` (verified `12.02`) for click-to-open Tile automation

Monk's is **not required**. v1.2.1 adds a native GM Push to Player workflow and generated Push Macro using Hexcode Breach Lite's own Foundry package socket. No compendium packs or audio assets are required.
...
5. Press **Save Puzzle**.
6. Choose either delivery path:
   - **With Monk's Active Tile Triggers:** select the terminal/device Tile, press **Bind Selected Tile**, create the Tile Helper Macro once, and use it from a Monk's **Run Macro** action.
   - **Without Monk's:** press **Save + Push to Player**, choose an online player/owned Netrunner, and Hexcode opens directly on that player's client.
7. For a hotbar-friendly dependency-free workflow, press **Create Push Macro** once. The generated Macro lets the GM choose any saved Scene or Portable breach plus an eligible online player at run time.

Both generated Macros are universal. You do **not** hard-code puzzle IDs into Tile actions or the Push Macro.
...
### Player-loop architecture (unchanged from v1.2.0)
...
## Native GM Push to Player

v1.2.1 provides a lightweight Foundry-native alternative to automated Tile activation:

- **Save + Push to Player** in GM Config saves the loaded puzzle, then opens a target selector.
- Only **online non-GM users** are listed.
- The selector prefers Netrunner Actors represented by **owned tokens on the source Scene**, then the player's assigned Netrunner character. Owned world Netrunners are used only as a fallback so the list stays useful instead of filling with unrelated Actors.
- The receiving client independently verifies that the sender is a GM, the Actor is owned by that player, the Actor still qualifies as a Netrunner, and the saved puzzle still exists in the requested Scene/Portable scope.
- The GM receives an acknowledgement when the player client opens the breach. If the player goes offline or no eligible Netrunner can be verified, the push fails cleanly instead.
- **Create Push Macro** creates/updates one universal `Hexcode Breach — Push to Player` Script Macro. Running it opens the same live selector for saved Scene and Portable puzzles.

This workflow uses the same `module.hexcode-breach-lite` package socket already used by GM-authoritative rewards; no extra module is required. Monk's Active Tile Triggers remains the recommended option for automatic click-to-open Tiles.
